...
use crate::errors::{BuiltinError, FileContext, IoErrorContextExt};
...
pub fn read_file(_shell: &mut Shell, console: &mut Console, args: Vec<&str>) -> Result<()> {
    use FileContext::FileRead;
    check_args(&args, 1, "read-file <path>", console)?;
    let file_name = PathBuf::from(args[0]);
    let file = fs_err::File::open(&file_name).file_context(FileRead)?;

    let reader = BufReader::new(file);
    for line in reader.lines() {
        let line = line.file_context(FileRead)?;
        showln!(console, "{}", &line);
    }
